...
**赤い攻撃**は通常のダッシュ回避では避けられず、ちゃんとかわすかチャージダッシュを使う必要がある
...
ボスを倒した際に、GameManager.instance.GameClear(int 各ボスの番号);が実行されるようにする。

ボスの番号　1:騎士　2:蜘蛛　3:リザードウォーリア　4:盾騎士

具体例：番号1の騎士が倒されたら　GameManager.instance.GameClear(1);が実行される
...
どのボスが討伐されているかの情報の保存。セーブの実装法を知らない
